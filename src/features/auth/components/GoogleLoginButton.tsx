"use client";

import { useState } from "react";
import { cn } from "@/lib";
import { env } from "@/lib/env";
import { API_ENDPOINTS } from "@/lib/api/endpoints";

interface GoogleLoginButtonProps {
    redirectTo?: string;
    variant?: "default" | "outline";
    size?: "sm" | "md" | "lg";
    className?: string;
}

/**
 * Google OAuth Login Button
 * Redirects user to backend Google OAuth endpoint
 *
 * @param redirectTo - URL to redirect after successful login (optional)
 * @param variant - Button style variant
 * @param size - Button size
 * @param className - Additional CSS classes
 */
export function GoogleLoginButton({
    redirectTo = "/",
    variant = "default",
    size = "md",
    className,
}: GoogleLoginButtonProps) {
    const [isLoading, setIsLoading] = useState(false);

    const handleGoogleLogin = () => {
        setIsLoading(true);

        const backendUrl =
            process.env.NODE_ENV === "production"
                ? env.NEXT_PUBLIC_API_BASE_URL
                : "http://localhost:5000";

        if (!backendUrl) {
            console.error("API Base URL is not configured");
            setIsLoading(false);
            return;
        }

        // Build OAuth URL with redirect parameter
        const oauthUrl = new URL(API_ENDPOINTS.auth.google, backendUrl);

        if (redirectTo && redirectTo !== "/") {
            oauthUrl.searchParams.set("redirect", redirectTo);
        }

        // Redirect to backend OAuth endpoint
        window.location.href = oauthUrl.toString();
    };

    // Size variants
    const sizeClasses = {
        sm: "px-3 py-1.5 text-sm",
        md: "px-4 py-2 text-base",
        lg: "px-6 py-3 text-lg",
    };

    // Style variants
    const variantClasses = {
        default:
            "bg-white border-2 border-gray-300 hover:bg-gray-50 text-gray-700 hover:border-gray-400",
        outline:
            "bg-transparent border-2 border-gray-300 hover:bg-gray-50 text-gray-700 hover:border-gray-400",
    };

    return (
        <button
            onClick={handleGoogleLogin}
            disabled={isLoading}
            className={cn(
                "w-full flex items-center justify-center gap-3 rounded-lg font-medium transition-all duration-200",
                "disabled:opacity-50 disabled:cursor-not-wait",
                "focus:outline-none focus:ring-2 focus:ring-blue-500 focus:ring-offset-2",
                sizeClasses[size],
                variantClasses[variant],
                className,
            )}
            aria-label='Sign in with Google'
        >
            {/* Google Logo SVG */}
            <svg
                className={cn(
                    "flex-shrink-0",
                    size === "sm" && "w-4 h-4",
                    size === "md" && "w-5 h-5",
                    size === "lg" && "w-6 h-6",
                )}
                viewBox='0 0 24 24'
                xmlns='http://www.w3.org/2000/svg'
            >
                <path
                    d='M22.56 12.25c0-.78-.07-1.53-.2-2.25H12v4.26h5.92c-.26 1.37-1.04 2.53-2.21 3.31v2.77h3.57c2.08-1.92 3.28-4.74 3.28-8.09z'
                    fill='#4285F4'
                />
                <path
                    d='M12 23c2.97 0 5.46-.98 7.28-2.66l-3.57-2.77c-.98.66-2.23 1.06-3.71 1.06-2.86 0-5.29-1.93-6.16-4.53H2.18v2.84C3.99 20.53 7.7 23 12 23z'
                    fill='#34A853'
                />
                <path
                    d='M5.84 14.09c-.22-.66-.35-1.36-.35-2.09s.13-1.43.35-2.09V7.07H2.18C1.43 8.55 1 10.22 1 12s.43 3.45 1.18 4.93l2.85-2.22.81-.62z'
                    fill='#FBBC05'
                />
                <path
                    d='M12 5.38c1.62 0 3.06.56 4.21 1.64l3.15-3.15C17.45 2.09 14.97 1 12 1 7.7 1 3.99 3.47 2.18 7.07l3.66 2.84c.87-2.6 3.3-4.53 6.16-4.53z'
                    fill='#EA4335'
                />
            </svg>

            <span className='font-medium'>
                {isLoading ? "Connecting to Google..." : "Continue with Google"}
            </span>

            {/* Loading Spinner */}
            {isLoading && (
                <svg
                    className='animate-spin h-4 w-4 text-gray-600'
                    xmlns='http://www.w3.org/2000/svg'
                    fill='none'
                    viewBox='0 0 24 24'
                >
                    <circle
                        className='opacity-25'
                        cx='12'
                        cy='12'
                        r='10'
                        stroke='currentColor'
                        strokeWidth='4'
                    />
                    <path
                        className='opacity-75'
                        fill='currentColor'
                        d='M4 12a8 8 0 018-8V0C5.373 0 0 5.373 0 12h4zm2 5.291A7.962 7.962 0 014 12H0c0 3.042 1.135 5.824 3 7.938l3-2.647z'
                    />
                </svg>
            )}
        </button>
    );
}
